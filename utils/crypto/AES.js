...
class AES {
    ...
    subBytes(byte) {
        const row = parseInt(byte[0], 16);
        const column = parseInt(byte[1], 16);

        return decToHex(AES.sBox[row][column]);
    }
    ...
    keyExpansion() {
        ...
        keySchedule.forEach((col, index) => {
            const ignoredIndex = [0, 1, 2, 3];

            if (ignoredIndex.includes(index)) {
                return;
            } else if (index % 4 === 0) {
                const lastColOfMatrix = new MatrixColumn(keySchedule[index - 1]).shiftUp().column.map(byte => {
                    return this.subBytes(byte);
                });
    ...
    subBytesRound(hexMatrix) {
        const subBytesResult = [];
        hexMatrix.forEach(row => {
            const subBytesRow = [];
            row.forEach(byte => {
                subBytesRow.push(this.subBytes(byte));
            });

            subBytesResult.push(subBytesRow);
        });
    ...
    mixColumnsRound(hexMatrix) {
        ...
        for (let i = 0; i < mixedColumns.length; i++) {
            ...
            for (let j = 0; j < mixedColumns[i].length; j++) {
                ...
                mixMatrixCol = new MatrixColumn(mixMatrixCol).convertValue('hex', 'dec');

                let galoisMult = new MatrixColumn(matrixCol.multiply(mixMatrixCol)).column.reduce((acc, curr) => acc ^ curr, 0);

                galoisMult = decToHex(galoisMult);
    ...
    encryptRound(matrix, roundKey) {
        ...
        if (roundKey === 10) {
            const shiftColumnMatrix = new Matrix(shiftRowsResult).convertValue('hex', 'bin');
            const lastRoundKeyMatrix = new Matrix(nthRoundKeySchedule).transpose().convertValue('hex', 'bin');
            const lastRoundResult = shiftColumnMatrix.xor(lastRoundKeyMatrix).convertValue('bin', 'hex').matrix;
            return lastRoundResult;
        }

        const mixColumnsResult = this.mixColumnsRound(shiftRowsResult);
        const mixColumnMatrix = new Matrix(mixColumnsResult).convertValue('hex', 'bin');
        const nthRoundKeyMatrix = new Matrix(nthRoundKeySchedule).transpose().convertValue('hex', 'bin');
        const nthRoundResult = mixColumnMatrix.xor(nthRoundKeyMatrix).convertValue('bin', 'hex').matrix;
        return nthRoundResult;
    }

    encrypt(plaintext) {
        const initialRoundResult = this.initialRound(plaintext);
        let roundResult = initialRoundResult;

        for (let i = 1; i <= 10; i++) {
            roundResult = this.encryptRound(roundResult, i);
        }

        const lastResult = new Matrix(roundResult).transpose().convertValue('hex', 'text').matrix;
        return lastResult;
    }
}

const aes = new AES();
const plaintext = 'ARIANDOHAREDISON';
// console.log(aes.keySchedule);
// console.log(aes.initialRound(plaintext));
console.log(aes.encrypt(plaintext));
